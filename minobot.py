...
DB_FAISS_PATH = 'vectorstore/db_faiss'  # Adjust this path as needed
...
st.title("💬 Mining Bot")
st.caption("🚀 A Chatbot powered by LLMA2")
...
for msg in st.session_state.messages:
    st.write(f"{msg['role']}: {msg['content']}")  # Updated message display
...
if record_audio:
    ...
    try:
        ...
        st.write(f"assistant: {msg['content']}")  # Updated message display
...
if prompt := st.text_input("User Input"):  # Changed to text_input
    st.session_state.messages.append({"role": "user", "content": prompt})
    st.write(f"user: {prompt}")  # Updated message display
    ...
    st.write(f"assistant: {translated_response}")  # Updated message display
    st.session_state.messages.append({"role": "assistant", "content": translated_response})

# Optional: Render stored responses and their download links
